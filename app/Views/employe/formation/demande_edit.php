...
<style>
    ...
    .badge-id {
        background: var(--e-primary-pale); border: 1px solid var(--e-primary-border);
        color: var(--e-primary); font-size: 0.7rem; font-weight: 700;
    ...
    .form-section-head {
        padding: 10px 16px; background: rgba(107,175,107,0.04);
        border-bottom: 1px solid var(--c-border);
        display: flex; align-items: center; gap: 8px;
        color: var(--e-primary); font-size: 0.78rem; font-weight: 700;
    ...
    .type-opt:hover    { border-color: var(--e-primary-border); }
    .type-opt.selected { border-color: var(--e-primary-border); background: var(--e-primary-pale); }
    ...
    .type-opt-icon {
        ...
        background: var(--e-primary-pale); border: 1px solid var(--e-primary-border);
        display: flex; align-items: center; justify-content: center;
        color: var(--e-primary); font-size: 0.75rem; flex-shrink: 0;
    }
    ...
    .type-opt.selected .type-opt-label { color: var(--e-primary); }
    ...
    .source-opt:hover    { border-color: var(--e-primary-border); }
    .source-opt.selected { border-color: var(--e-primary-border); background: var(--e-primary-pale); }
    ...
    .source-opt.selected .source-opt-label { color: var(--e-primary); }
    ...
    .frm-preview-title { color: #fff; font-size: 0.83rem; font-weight: 700; margin-bottom: 8px; }
...
<?php
$d = $demande;
$currentSource = !empty($d['id_Frm']) ? 'catalogue' : 'libre';
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-pen me-2" style="color:var(--e-primary);"></i>Modifier la demande</h1>
        <p>Demande #<?= (int)$d['id_DFrm'] ?></p>
    </div>
    <a href="<?= base_url('demande-formation/show/' . $d['id_DFrm']) ?>" class="btn-ghost">
...
<div class="form-card">
    <div class="form-card-header">
        <div class="form-card-header-left">
            ...
            <div>
                <p class="form-card-title">Modifier la demande</p>
                <p class="form-card-subtitle">Seules les demandes en attente peuvent être modifiées</p>
            </div>
        </div>
        <span class="badge-id"># <?= (int)$d['id_DFrm'] ?></span>
    </div>

    <form action="<?= base_url('demande-formation/update/' . $d['id_DFrm']) ?>" method="POST">
        <?= csrf_field() ?>
            ...
            <div class="form-section">
                ...
                <div class="form-section-body">
                    <div class="type-toggle">
                        <?php $curType = old('Type_DFrm', $d['Type_DFrm']); ?>
                        <label class="type-opt <?= $curType === 'demande' ? 'selected' : '' ?>"
                               id="type-opt-demande" onclick="selectType('demande')">
                            <input type="radio" name="Type_DFrm" value="demande"
                                   <?= $curType === 'demande' ? 'checked' : '' ?>>
                            <div class="type-opt-icon"><i class="fas fa-hand-paper"></i></div>
                            <div>
                                <div class="type-opt-label">Demande</div>
                                <div class="type-opt-desc">Je souhaite participer</div>
                            </div>
                        </label>
                        <label class="type-opt <?= $curType === 'invitation' ? 'selected' : '' ?>"
                               id="type-opt-invitation" onclick="selectType('invitation')">
                            <input type="radio" name="Type_DFrm" value="invitation"
                                   <?= $curType === 'invitation' ? 'checked' : '' ?>>
            ...
            <div class="form-section">
                ...
                <div class="form-section-body">
                    <div class="source-toggle">
                        <?php $curSrc = old('_source', $currentSource); ?>
                        <label class="source-opt <?= $curSrc === 'catalogue' ? 'selected' : '' ?>"
                               id="src-opt-catalogue" onclick="selectSource('catalogue')">
                            <input type="radio" name="_source" value="catalogue"
                                   <?= $curSrc === 'catalogue' ? 'checked' : '' ?>>
                        ...
                        <label class="source-opt <?= $curSrc === 'libre' ? 'selected' : '' ?>"
                               id="src-opt-libre" onclick="selectSource('libre')">
                            <input type="radio" name="_source" value="libre"
                                   <?= $curSrc === 'libre' ? 'checked' : '' ?>>
                    ...
                    <!-- Catalogue -->
                    <div class="source-panel <?= $curSrc === 'catalogue' ? 'visible' : '' ?>" id="src-panel-catalogue">
                        <div class="form-group">
                            <label class="form-label">Formation <span class="req">*</span></label>
                            <select name="id_Frm" class="form-control" id="sel-formation"
                                    onchange="previewFormation()">
                                <option value="">— Choisir dans le catalogue —</option>
                                <?php
                                $selFrm = old('id_Frm', $d['id_Frm'] ?? '');
                                foreach ($formations as $frm): ?>
                                <option value="<?= $frm['id_Frm'] ?>"
                                        <?= $selFrm == $frm['id_Frm'] ? 'selected' : '' ?>>
                    ...
                    <!-- Libre -->
                    <div class="source-panel <?= $curSrc === 'libre' ? 'visible' : '' ?>" id="src-panel-libre">
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">Intitulé <span class="req">*</span></label>
                                <input type="text" name="Description_Libre" class="form-control"
                                       placeholder="Intitulé de la formation"
                                       value="<?= old('Description_Libre', esc($d['Description_Libre'] ?? '')) ?>">
                            </div>
                        </div>
                        <div class="form-row form-row-2">
                            <div class="form-group">
                                ...
                                <input type="date" name="DateDebut_Libre" class="form-control"
                                       value="<?= old('DateDebut_Libre', $d['DateDebut_Libre'] ?? '') ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Date de fin</label>
                                <input type="date" name="DateFin_Libre" class="form-control"
                                       value="<?= old('DateFin_Libre', $d['DateFin_Libre'] ?? '') ?>">
                            </div>
                        </div>
                        <div class="form-row form-row-2">
                            <div class="form-group">
                ...
                <div class="form-section-body">
                    <div class="form-group">
                        ...
                        <textarea name="Motif" class="form-control" rows="4"
                                  placeholder="Expliquez votre motivation..."><?= old('Motif', esc($d['Motif'] ?? '')) ?></textarea>
                    </div>
                </div>
            </div>
