...
<style>
    ...
    .info-badge { background:rgba(245,166,35,0.1); border:1px solid rgba(245,166,35,0.2); color:#F5A623; border-radius:10px; padding:10px 16px; font-size:0.82rem; margin-bottom:20px; display:flex; align-items:center; gap:8px; }
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-pen me-2" style="color:var(--c-primary);"></i>Modifier mon absence</h1>
        <p>Référence #<?= $absence['id_Abs'] ?></p>
    </div>
    <a href="<?= base_url('absence/show/'.$absence['id_Abs']) ?>" class="btn-back"><i class="fas fa-arrow-left"></i> Retour</a>
</div>
...
<div class="form-card">
    <div class="form-head">
        ...
        <h5>Modification de la déclaration</h5>
    </div>
    <form method="post" action="<?= base_url('absence/update/'.$absence['id_Abs']) ?>">
        <?= csrf_field() ?>
        <div class="form-body">
            <div class="form-row">
                ...
                <select name="id_TAbs" class="form-select-dark" required>
                    ...
                    <option value="<?= $t['id_TAbs'] ?>" <?= $absence['id_TAbs'] == $t['id_TAbs'] ? 'selected' : '' ?>>
                        <?= esc($t['Libelle_TAbs']) ?>
                    </option>
                    <?php endforeach; ?>
                </select>
                ...
                <div class="col-6">
                    <div class="form-row">
                        ...
                        <input type="date" name="DateDebut_Abs" class="form-control-dark"
                            value="<?= esc(old('DateDebut_Abs', $absence['DateDebut_Abs'])) ?>" required>
                    </div>
                </div>
                <div class="col-6">
                    <div class="form-row">
                        ...
                        <input type="date" name="DateFin_Abs" class="form-control-dark"
                            value="<?= esc(old('DateFin_Abs', $absence['DateFin_Abs'])) ?>">
                    </div>
                </div>
            </div>
            <div class="form-row">
                <label class="form-label-dark">Motif</label>
                <textarea name="Motif_Abs" class="form-control-dark" rows="3"><?= esc(old('Motif_Abs', $absence['Motif_Abs'])) ?></textarea>
            </div>
            <div class="form-row">
                <label class="form-label-dark">Rapport</label>
                <textarea name="Rapport_Abs" class="form-control-dark" rows="3"><?= esc(old('Rapport_Abs', $absence['Rapport_Abs'])) ?></textarea>
            </div>
        </div>
        <div class="form-foot">
            <button type="submit" class="btn-submit"><i class="fas fa-save"></i> Enregistrer</button>
            <a href="<?= base_url('absence/show/'.$absence['id_Abs']) ?>" class="btn-back">Annuler</a>
        </div>
    </form>
</div>
