...
<style>
    ...
    .filter-ctrl:focus { border-color:var(--c-primary-border); }
    ...
    .notif-item.unread       { background:rgba(58,123,213,0.05); }
    .notif-item.unread:hover { background:rgba(58,123,213,0.08); }
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-bell me-2" style="color:#3A7BD5;"></i>Mes notifications</h1>
...
<div class="stats-row">
    <div class="stat-pill">
        <div class="stat-pill-icon" style="background:var(--c-primary-pale);border:1px solid var(--c-primary-border);">
            <i class="fas fa-bell" style="color:var(--c-accent);"></i>
        </div>
        <div>
...
<div class="card">
    <div class="card-header">
        <div class="card-header-left">
            ...
            <p class="card-title">Toutes mes notifications</p>
        </div>
    ...
    <div class="notif-list" id="notif-list">
        ...
        <div class="notif-item <?= $unread ? 'unread' : '' ?>"
            ...
            <div class="notif-actions">
                ...
                <?php endif; ?>
                <?php if ($unread): ?>
                <button class="btn-icon btn-icon-primary" title="Marquer comme lue"
                        onclick="marquerLue(<?= $n['id_Notif'] ?>)">
...
(function () {
    ...
    window.filterNotifs = function () {
        ...
        if (btnReset) btnReset.style.display = (search || type || statut) ? '' : 'none';
    ...
    window.marquerLue = function (id) {
        fetch('<?= base_url('notifications/lire/') ?>' + id, {
            ...
            if (dot) dot.remove();
            item.querySelectorAll('.btn-icon-primary').forEach(function (b) {
    ...
    window.supprimerNotif = function (id) {
        ...
        fetch('<?= base_url('notifications/delete/') ?>' + id, {
            ...
            if (item) {
                ...
                setTimeout(function () { item.remove(); majCompteurs(); }, 200);
            }
        });
    };
