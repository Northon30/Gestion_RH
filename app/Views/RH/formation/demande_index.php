...
<style>
    ...
    .main-tab-btn.active .main-tab-count { background: var(--c-orange-border); color: var(--c-orange); }
    ...
    .dmd-table tbody tr:hover td { background: rgba(245,166,35,0.02); }
...
$mesDemandes   = array_values(array_filter($demandes, fn($d) => $d['id_Emp'] == $idEmp));
$toutesDemandes = $demandes;

$total      = count($toutesDemandes);
$enAttente  = count(array_filter($toutesDemandes, fn($d) => $d['Statut_DFrm'] === 'en_attente'));
$approuve   = count(array_filter($toutesDemandes, fn($d) => $d['Statut_DFrm'] === 'approuve'));
$valideRH   = count(array_filter($toutesDemandes, fn($d) => $d['Statut_DFrm'] === 'valide_rh'));
$rejetes    = count(array_filter($toutesDemandes, fn($d) => in_array($d['Statut_DFrm'], ['rejete','rejete_rh'])));
?>

<div class="page-header">
    <div>
        <h1><i class="fas fa-file-alt me-2" style="color:#F5A623;"></i>Demandes de formation</h1>
        <p><?= date('d/m/Y') ?></p>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <a href="<?= base_url('formation') ?>" class="btn-orange" style="background:rgba(91,155,240,0.15);border:1px solid rgba(91,155,240,0.3);color:#5B9BF0;">
            <i class="fas fa-chalkboard-teacher"></i> Catalogue
        </a>
        <a href="<?= base_url('demande-formation/create') ?>" class="btn-orange">
...
<!-- Onglets -->
<div class="main-tabs">
    <button class="main-tab-btn active" onclick="switchTab('mes', this)">
        <i class="fas fa-user"></i>
        Mes demandes
        <span class="main-tab-count"><?= count($mesDemandes) ?></span>
    </button>
    <?php if ($idPfl != 3): ?>
    <button class="main-tab-btn" onclick="switchTab('tous', this)">
        <i class="fas fa-users"></i>
        Toutes les demandes
        <span class="main-tab-count"><?= count($toutesDemandes) ?></span>
    </button>
    <?php endif; ?>
</div>

<?php
function renderDemandeTable(array $rows, string $prefix, int $idPfl, int $idEmp, array $directions): void
{
    $isTous  = ($prefix === 'tous');
    $colspan = ($isTous && $idPfl != 3) ? 9 : 8;
?>
<!-- Filtres -->
        ...
        <div class="filter-row">
            <div class="filter-group">
                ...
                <div class="filter-search-wrap">
                    ...
                    <input type="text" class="filter-input" id="<?= $prefix ?>-f-search"
                           placeholder="Nom, formation, motif...">
                </div>
            </div>
            <div class="filter-group">
            ...
            <div class="filter-group">
                ...
                <select class="filter-select" id="<?= $prefix ?>-f-annee">
                    <option value="">Toutes</option>
                    <?php
                    $annees = array_unique(array_map(fn($d) => date('Y', strtotime($d['DateDemande'])), $rows));
                    rsort($annees);
                    foreach ($annees as $a): ?>
    ...
    <table class="dmd-table">
        ...
        <tbody id="<?= $prefix ?>-tbody">
            <?php foreach ($rows as $i => $d):
                ...
                $canEdit   = ($d['id_Emp'] == $idEmp && $statut === 'en_attente');
                $canDelete = ($d['id_Emp'] == $idEmp && $statut === 'en_attente');
            ?>
            <tr class="<?= $prefix ?>-row"
                data-nom="<?= strtolower(esc(($d['Nom_Emp'] ?? '') . ' ' . ($d['Prenom_Emp'] ?? ''))) ?>"
                data-desc="<?= strtolower(esc($desc . ' ' . $motif)) ?>"
                data-statut="<?= esc($statut) ?>"
                data-type="<?= esc($d['Type_DFrm'] ?? '') ?>"
                data-dir="<?= esc($d['Nom_Dir'] ?? '') ?>"
                ...
                <td>
                    <div class="emp-identity">
                        <div class="emp-avatar">
                            <?= mb_substr($d['Nom_Emp'] ?? '?', 0, 1) . mb_substr($d['Prenom_Emp'] ?? '', 0, 1) ?>
                        </div>
                        <div>
                            <p class="emp-name"><?= esc(($d['Nom_Emp'] ?? '') . ' ' . ($d['Prenom_Emp'] ?? '')) ?></p>
                            <?php if (!empty($d['Nom_Dir'])): ?>
                            <div class="emp-dir"><?= esc($d['Nom_Dir']) ?></div>
                            <?php endif; ?>
                        </div>
                    </div>
                </td>
...
<div class="main-tab-panel active" id="panel-mes">
    <?php renderDemandeTable($mesDemandes, 'mes', $idPfl, $idEmp, $directions); ?>
</div>

<!-- Panel Toutes les demandes -->
<?php if ($idPfl != 3): ?>
<div class="main-tab-panel" id="panel-tous">
    <?php renderDemandeTable($toutesDemandes, 'tous', $idPfl, $idEmp, $directions); ?>
</div>
<?php endif; ?>
...
<div class="modal-overlay" id="modal-delete">
    <div class="modal-box">
        ...
        <p style="color:var(--c-muted);font-size:0.8rem;text-align:center;line-height:1.5;margin:0;">
            Vous allez supprimer la demande :<br>
...
(function () {
    ...
    function buildFilter(prefix) {
        ...
        function applyFilters() {
            ...
            var actifs = [search, statut, type, dir, annee, demandeFrom, demandeTo, debutFrom, debutTo]
            ...
            rows.forEach(function (row) {
                var match =
                    (search      === '' || row.dataset.nom.includes(search) || row.dataset.desc.includes(search)) &&
        ...
        window['resetFilters'] = function (p) {
            ...
            [prefix+'-f-search', prefix+'-f-statut', prefix+'-f-type', prefix+'-f-dir',
             prefix+'-f-annee', prefix+'-f-demande-from', prefix+'-f-demande-to',
             prefix+'-f-debut-from', prefix+'-f-debut-to']
