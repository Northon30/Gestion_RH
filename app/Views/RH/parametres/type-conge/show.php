...
<style>
    ...
    .statut-approuve_chef   { background:rgba(58,123,213,0.12); border:1px solid rgba(58,123,213,0.3); color:#5B9BF0; }
    .statut-rejete_chef     { background:var(--c-red-pale);     border:1px solid var(--c-red-border);  color:#ff8080; }
...
$libelles = [
    ...
    'approuve_chef' => ['label' => 'Approuvé Chef',   'icon' => 'fa-user-check'],
    'rejete_chef'   => ['label' => 'Rejeté Chef',     'icon' => 'fa-user-times'],
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-tags me-2" style="color:#F5A623;"></i><?= esc($type['Libelle_Tcg']) ?></h1>
        <p><?= $nbConges ?> congé(s) associé(s)</p>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <a href="<?= base_url('parametres/type-conge') ?>" class="btn-ghost">
            <i class="fas fa-arrow-left"></i> Retour
        </a>
        <a href="<?= base_url('parametres/type-conge/edit/' . $type['id_Tcg']) ?>" class="btn-orange">
...
<div class="show-grid">
    ...
    <div class="card">
        <div class="type-hero">
            ...
            <div>
                <div class="type-hero-name"><?= esc($type['Libelle_Tcg']) ?></div>
                <div class="type-hero-id">ID # <?= (int)$type['id_Tcg'] ?></div>
            </div>
        </div>
        ...
        <div class="info-body">
            <div class="info-row">
                ...
                <span class="info-value" style="font-weight:600;"><?= esc($type['Libelle_Tcg']) ?></span>
            </div>
            <div class="info-row">
                <span class="info-label">Identifiant</span>
                <span class="info-value"><?= (int)$type['id_Tcg'] ?></span>
            </div>
            <div class="info-row">
                <span class="info-label">Congés associés</span>
    ...
    <!-- COL DROITE — Congés associés -->
    <div class="card">
        <div class="card-header">
            <div class="card-icon"><i class="fas fa-calendar-alt"></i></div>
            <p class="card-title">Congés avec ce type</p>
            <span class="badge-count"><?= $nbConges ?></span>
        </div>

        <?php if (empty($conges)): ?>
        <div class="empty-state">
            <i class="fas fa-calendar-alt"></i>
            Aucun congé n'utilise ce type pour l'instant.
        </div>
        <?php else: ?>
        <table class="conge-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:var(--c-text);"><?= esc($c['Libelle_Cge'] ?? '-') ?></td>
