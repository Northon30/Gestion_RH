...
<style>
    ...
    .profil-name:hover { color:var(--c-orange); }
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-id-badge me-2" style="color:#F5A623;"></i>Profils</h1>
        <p>Gestion des profils d'accès</p>
    </div>
    <a href="<?= base_url('parametres/profil/create') ?>" class="btn-orange">
...
<div class="stats-row">
    <div class="stat-pill">
        <div class="stat-pill-icon" style="background:var(--c-orange-pale);border:1px solid var(--c-orange-border);">
            <i class="fas fa-id-badge" style="color:var(--c-orange);"></i>
        </div>
        <div>
            <div class="stat-pill-val"><?= $total ?></div>
            <div class="stat-pill-lbl">Profils</div>
        </div>
    </div>
    <div class="stat-pill">
        ...
        <div>
            <div class="stat-pill-val" style="color:#5B9BF0;"><?= $totalEmployes ?></div>
            <div class="stat-pill-lbl">Employés associés</div>
        </div>
    </div>
    <div class="stat-pill">
...
<div class="card">
    <div class="card-header">
        <div class="card-header-left">
            ...
            <p class="card-title">Liste des profils</p>
        </div>
        <span class="card-count" id="profil-count"><?= $total ?></span>
    </div>
    ...
    <div class="filter-bar">
        <div class="filter-group">
            ...
            <input type="text" id="f-search" class="filter-ctrl"
                   placeholder="Libellé..." style="width:200px;"
                   oninput="filterProfils()">
        </div>
        <div class="filter-group">
            <label class="filter-lbl">Utilisation</label>
            <select id="f-usage" class="filter-ctrl" onchange="filterProfils()">
    ...
    <?php else: ?>
    <table class="data-table" id="profil-table">
        <thead>
            <tr>
                <th>#</th>
                <th>Libellé</th>
        ...
        <tbody id="profil-tbody">
            <?php foreach ($profils as $i => $g): ?>
            <tr class="profil-row"
                data-libelle="<?= strtolower(esc($g['Libelle_Pfl'])) ?>"
                data-usage="<?= $g['nb_employes'] > 0 ? 'used' : 'unused' ?>">
                <td class="row-num" style="color:var(--c-muted);font-size:0.7rem;"><?= $i + 1 ?></td>
                <td>
                    <a href="<?= base_url('parametres/profil/show/' . $g['id_Pfl']) ?>"
                       class="profil-name">
                        <?= esc($g['Libelle_Pfl']) ?>
                    </a>
                </td>
                <td style="text-align:center;">
                    <span class="card-count"><?= (int)$g['nb_employes'] ?></span>
                </td>
                <td>
                    <?php if ($g['nb_employes'] > 0): ?>
                    <span class="badge-used">
                        <i class="fas fa-check"></i> Utilisé
                    </span>
                    <?php else: ?>
                    <span class="badge-unused">
                ...
                <td style="text-align:center;">
                    <div style="display:inline-flex;gap:5px;">
                        ...
                        <a href="<?= base_url('parametres/profil/edit/' . $g['id_Pfl']) ?>"
                           class="btn-icon btn-icon-orange" title="Modifier">
                            <i class="fas fa-pen"></i>
                        </a>
                        <button class="btn-icon btn-icon-red" title="Supprimer"
                                onclick="confirmDelete(<?= $g['id_Pfl'] ?>, '<?= esc($g['Libelle_Pfl'], 'js') ?>', <?= (int)$g['nb_employes'] ?>)">
    ...
    <div id="no-match" style="display:none;" class="empty-state">
        <i class="fas fa-search"></i> Aucun profil ne correspond aux filtres.
    </div>

    <div class="table-footer" id="table-footer">
        <?= $total ?> profil(s) au total
    </div>
    <?php endif; ?>
</div>
...
(function () {
    ...
    window.filterProfils = function () {
        ...
        var rows    = document.querySelectorAll('.profil-row');
        var visible = 0;
        ...
        var counter = document.getElementById('profil-count');

        if (noMatch) noMatch.style.display = visible === 0 && rows.length > 0 ? 'block' : 'none';
        if (footer)  footer.textContent    = visible + ' profil(s) affiché(s)';
    ...
    // ===== MODAL SUPPRESSION =====
    window.confirmDelete = function (id, label, nbEmployes) {
        ...
        if (nbEmployes > 0) {
            msg.innerHTML = 'Impossible de supprimer <strong style="color:#fff;">' + label
                + '</strong> : ' + nbEmployes + ' employé(s) y sont associés.';
            form.innerHTML = '';
            var btnAnnuler = document.createElement('button');
            btnAnnuler.type = 'button';
            btnAnnuler.onclick = closeDeleteModal;
            btnAnnuler.style.cssText = 'width:100%;background:transparent;border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.55);font-weight:600;border-radius:8px;padding:9px 16px;font-size:0.82rem;cursor:pointer;';
            btnAnnuler.innerHTML = '<i class="fas fa-times"></i> Fermer';
            form.appendChild(btnAnnuler);
        } else {
            msg.innerHTML = 'Supprimer le profil <strong style="color:#fff;">' + label + '</strong> ?';
            form.innerHTML = '<?= csrf_field() ?><button type="submit" style="width:100%;background:linear-gradient(135deg,#c0392b,#922b21);border:none;color:#fff;font-weight:700;border-radius:8px;padding:9px 16px;font-size:0.82rem;cursor:pointer;"><i class=\'fas fa-trash-alt\'></i> Supprimer</button>';
            form.action = '<?= base_url('parametres/profil/delete/') ?>' + id;
        }
