...
<style>
    ...
    .btn-danger:hover { background:var(--c-red-pale); }
    ...
    .table-footer { padding:10px 16px; border-top:1px solid var(--c-border); font-size:0.73rem; color:var(--c-muted); }
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-calendar-alt me-2" style="color:#F5A623;"></i>Types d'événements</h1>
        <p>Gestion des types d'événements</p>
    </div>
    <a href="<?= base_url('parametres/type-evenement/create') ?>" class="btn-orange">
...
<div class="stats-row">
    ...
    <div class="stat-pill">
        <div class="stat-pill-icon" style="background:var(--c-blue-pale);border:1px solid var(--c-blue-border);">
            <i class="fas fa-calendar-check" style="color:#5B9BF0;"></i>
        </div>
        <div>
            <div class="stat-pill-val" style="color:#5B9BF0;"><?= $totalEvenements ?></div>
            <div class="stat-pill-lbl">Événements associés</div>
        </div>
    </div>
    <div class="stat-pill">
...
<div class="card">
    <div class="card-header">
        <div class="card-header-left">
            ...
            <p class="card-title">Liste des types d'événements</p>
        </div>
    ...
    <div class="empty-state">
        ...
        Aucun type d'événement enregistré.
    </div>
    <?php else: ?>
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr class="type-row"
                data-libelle="<?= strtolower(esc($t['Libelle_Tev'])) ?>"
                data-usage="<?= $t['nb_evenements'] > 0 ? 'used' : 'unused' ?>">
                <td class="row-num" style="color:var(--c-muted);font-size:0.7rem;"><?= $i + 1 ?></td>
                <td>
                    <a href="<?= base_url('parametres/type-evenement/show/' . $t['id_Tev']) ?>"
                       class="type-name"><?= esc($t['Libelle_Tev']) ?></a>
                </td>
                <td style="text-align:center;">
                    <span class="card-count"><?= (int)$t['nb_evenements'] ?></span>
                </td>
                <td>
                    <?php if ($t['nb_evenements'] > 0): ?>
                ...
                <td style="text-align:center;">
                    <div style="display:inline-flex;gap:5px;">
                        ...
                        <a href="<?= base_url('parametres/type-evenement/edit/' . $t['id_Tev']) ?>"
                           class="btn-icon btn-icon-orange" title="Modifier">
                            <i class="fas fa-pen"></i>
                        </a>
                        <button class="btn-icon btn-icon-red" title="Supprimer"
                                onclick="confirmDelete(<?= $t['id_Tev'] ?>, '<?= esc($t['Libelle_Tev'], 'js') ?>', <?= (int)$t['nb_evenements'] ?>)">
...
(function () {
    ...
    window.filterTypes = function () {
        ...
        btnReset.style.display = (search || usage) ? '' : 'none';
    ...
    window.confirmDelete = function (id, label, nbEvenements) {
        ...
        if (nbEvenements > 0) {
            msg.innerHTML = 'Impossible de supprimer <strong style="color:#fff;">' + label
                + '</strong> : ' + nbEvenements + ' événement(s) y sont associés.';
            ...
            form.action = '<?= base_url('parametres/type-evenement/delete/') ?>' + id;
        }
