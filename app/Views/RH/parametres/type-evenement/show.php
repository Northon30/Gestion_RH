...
<?php $nbEvenements = count($evenements); ?>

<div class="page-header">
    <div>
        <h1><i class="fas fa-calendar-alt me-2" style="color:#F5A623;"></i><?= esc($type['Libelle_Tev']) ?></h1>
        <p><?= $nbEvenements ?> événement(s) associé(s)</p>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <a href="<?= base_url('parametres/type-evenement') ?>" class="btn-ghost">
            <i class="fas fa-arrow-left"></i> Retour
        </a>
        <a href="<?= base_url('parametres/type-evenement/edit/' . $type['id_Tev']) ?>" class="btn-orange">
...
<div class="show-grid">
    ...
    <!-- COL GAUCHE — Infos type -->
    <div class="card">
        <div class="type-hero">
            <div class="type-hero-icon"><i class="fas fa-calendar-alt"></i></div>
            <div>
                <div class="type-hero-name"><?= esc($type['Libelle_Tev']) ?></div>
                <div class="type-hero-id">ID # <?= (int)$type['id_Tev'] ?></div>
            </div>
        </div>
        ...
        <div class="info-body">
            <div class="info-row">
                ...
                <span class="info-value" style="font-weight:600;"><?= esc($type['Libelle_Tev']) ?></span>
            </div>
            <div class="info-row">
                <span class="info-label">Identifiant</span>
                <span class="info-value"><?= (int)$type['id_Tev'] ?></span>
            </div>
            <div class="info-row">
                <span class="info-label">Événements associés</span>
    ...
    <!-- COL DROITE — Événements associés -->
    <div class="card">
        <div class="card-header">
            <div class="card-icon"><i class="fas fa-calendar-check"></i></div>
            <p class="card-title">Événements avec ce type</p>
            <span class="badge-count"><?= $nbEvenements ?></span>
        </div>

        <?php if (empty($evenements)): ?>
        <div class="empty-state">
            <i class="fas fa-calendar-alt"></i>
            Aucun événement n'utilise ce type pour l'instant.
        </div>
        <?php else: ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:0.78rem;white-space:nowrap;">
                        <i class="fas fa-calendar me-1" style="color:var(--c-orange);font-size:0.7rem;"></i>
                        <?= date('d/m/Y', strtotime($e['Date_Evt'])) ?>
                    </td>
                    <td style="text-align:center;">
                        <span style="background:var(--c-blue-pale);border:1px solid var(--c-blue-border);
                                     color:#5B9BF0;border-radius:10px;padding:2px 9px;
                                     font-size:0.68rem;font-weight:700;">
                            <?= (int)$e['nb_participants'] ?>
                        </span>
                    </td>
                </tr>
                <?php endforeach; ?>
