...
<div class="page-header">
    <div>
        <h1><i class="fas fa-pen me-2" style="color:#F5A623;"></i>Modifier le type d'absence</h1>
        <p><?= esc($type['Libelle_TAbs']) ?></p>
    </div>
    <a href="<?= base_url('parametres/type-absence/show/' . $type['id_TAbs']) ?>" class="btn-ghost">
    ...
    <div class="form-card-header">
        <div class="form-card-header-left">
            ...
            <div>
                ...
                <p class="form-card-subtitle">Type absence ID # <?= (int)$type['id_TAbs'] ?></p>
            </div>
        </div>
        <span class="badge-id"># <?= (int)$type['id_TAbs'] ?></span>
    </div>

    <form action="<?= base_url('parametres/type-absence/update/' . $type['id_TAbs']) ?>" method="POST">
        <?= csrf_field() ?>
        <div class="form-card-body">
            <div class="current-val">
                <i class="fas fa-history" style="color:var(--c-orange);margin-right:6px;"></i>
                Valeur actuelle : <span><?= esc($type['Libelle_TAbs']) ?></span>
            </div>
            <div class="form-group">
                <label class="form-label">Nouveau libellé <span class="req">*</span></label>
                <input type="text" name="Libelle_TAbs" id="f-libelle"
                       class="form-control" maxlength="100"
                       value="<?= old('Libelle_TAbs', esc($type['Libelle_TAbs'])) ?>"
