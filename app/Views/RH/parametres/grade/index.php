...
<style>
    ...
    .grade-name:hover { color:var(--c-orange); }
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-medal me-2" style="color:#F5A623;"></i>Grades</h1>
        <p>Gestion des grades des employés</p>
    </div>
    <a href="<?= base_url('parametres/grade/create') ?>" class="btn-orange">
...
<div class="stats-row">
    <div class="stat-pill">
        <div class="stat-pill-icon" style="background:var(--c-orange-pale);border:1px solid var(--c-orange-border);">
            <i class="fas fa-medal" style="color:var(--c-orange);"></i>
        </div>
        <div>
            <div class="stat-pill-val"><?= $total ?></div>
            <div class="stat-pill-lbl">Grades</div>
        </div>
    </div>
    <div class="stat-pill">
...
<div class="card">
    <div class="card-header">
        <div class="card-header-left">
            ...
            <p class="card-title">Liste des grades</p>
        </div>
        <span class="card-count" id="grade-count"><?= $total ?></span>
    </div>
    ...
    <div class="filter-bar">
        <div class="filter-group">
            ...
            <input type="text" id="f-search" class="filter-ctrl"
                   ...
                   oninput="filterGrades()">
        </div>
        <div class="filter-group">
            <label class="filter-lbl">Utilisation</label>
            <select id="f-usage" class="filter-ctrl" onchange="filterGrades()">
            ...
            <tr class="grade-row"
                ...
                <td style="text-align:center;">
                    <div style="display:inline-flex;gap:5px;">
                        ...
                        <a href="<?= base_url('parametres/grade/edit/' . $g['id_Grd']) ?>"
                           class="btn-icon btn-icon-orange" title="Modifier">
                            <i class="fas fa-pen"></i>
                        </a>
                        <button class="btn-icon btn-icon-red" title="Supprimer"
                                onclick="confirmDelete(<?= $g['id_Grd'] ?>, '<?= esc($g['Libelle_Grd'], 'js') ?>', <?= (int)$g['nb_employes'] ?>)">
    ...
    <div id="no-match" style="display:none;" class="empty-state">
        <i class="fas fa-search"></i> Aucun grade ne correspond aux filtres.
    </div>

    <div class="table-footer" id="table-footer">
        <?= $total ?> grade(s) au total
    </div>
    <?php endif; ?>
</div>
...
(function () {
    ...
    window.filterGrades = function () {
        ...
        var counter = document.getElementById('grade-count');

        if (noMatch) noMatch.style.display = visible === 0 && rows.length > 0 ? 'block' : 'none';
        if (footer)  footer.textContent    = visible + ' grade(s) affiché(s)';
    ...
    window.confirmDelete = function (id, label, nbEmployes) {
        ...
        if (nbEmployes > 0) {
            ...
            msg.innerHTML = 'Supprimer le grade <strong style="color:#fff;">' + label + '</strong> ?';
            ...
            form.action = '<?= base_url('parametres/grade/delete/') ?>' + id;
        }
